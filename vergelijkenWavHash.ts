...
Then('moet het overeenkomen met het bestand in mijn workspace', () => {
  ...
  const downloadedFileHash = calculateHash(downloadedFilePath);
  const workspaceFileHash = calculateHash(workspaceFilePath);

  expect(downloadedFileHash).to.equal(workspaceFileHash);
});
